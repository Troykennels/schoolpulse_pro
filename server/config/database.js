...
async function runMigrations() {
  try {
    ...
    if (isSQLite) {
      // SQLite: run Knex migrations normally
    ...
    // PostgreSQL: use raw SQL — works with pooler, handles existing tables
    await db.raw(`
    ...
    const pending = ['001_initial_schema.js', '002_enhanced_modules.js']
      .filter(m => !done.includes(m));
    ...
    // Run only pending migration files
